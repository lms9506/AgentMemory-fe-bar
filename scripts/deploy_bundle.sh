...
export DATABRICKS_CONFIG_PROFILE="${DATABRICKS_PROFILE:-fevm-serverless-stable-2eacei}"
...
echo "  https://wealth-advisor-${TARGET}-$(databricks current-user me \
  -p "${DATABRICKS_CONFIG_PROFILE}" -o json 2>/dev/null \
...
echo "Or run a job:"
echo "  databricks bundle run wealth_advisor --target ${TARGET}"

...
- 2026-08-28 — **Batch bulk-onboarding via Lakeflow + Genie (ADR-0019, ADR-0020)**: added the batch ingest surface for the FE-bar certification (end-to-end journey: raw → Lakeflow → UC → Lakebase → distill → Genie → App). New Lakeflow declarative pipeline (`databricks/pipelines/dossier_ingest.py`: Auto Loader binaryFile bronze → `ai_parse_document`+`ai_query` silver), `hydrate` wheel task (`jobs/hydrate_entry.py`, loads Lakebase pgvector from silver reusing store/embed code), and `dossier_ingest_job` chaining pipeline→hydrate→distill. Synthetic landing seeder (`scripts/seed_landing.py`) stages personas' mixed-format fixtures + generated text clients (`client_1000+`, new `start_index` on the generator). Genie space over the governed Delta tables (`databricks/genie/dossier_space.json` + `scripts/setup_genie.py`). Notebook `03_bulk_onboard.py` orchestrates the run; `evidence/` holds text-readable execution evidence + a generator. Shared batch helpers in `memory/landing.py`; pipeline parser parity guarded by `tests/test_pipeline_parser.py`. **Built offline; needs a live serverless workspace for `bundle deploy` + evidence capture (old FEVM `agent-memos` workspace expired).**

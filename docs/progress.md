...
- 2026-07-27 — **First live end-to-end deploy on a fresh workspace — 9 blocking bugs fixed (NFR-2)**: deployed to a clean workspace (`fevm-serverless-stable-agent-memo`) from a fresh clone and drove the full path (`deploy_bundle.sh dev` → 00_setup → 01_seed → 02_demo) to a **running app with the 3 seeded demo clients** and a working FM chat/recall response. The prior "single-source config" work (ADR-0016) had **never been deployed live** and hid nine deploy/setup blockers, all now fixed on branch `lm/bundle-create-schema` (full write-up: `docs/deployment-findings.md`): (1) `build_frontend.sh` hard-failed when npm was present but the registry unreachable — now falls back to the shipped `dist/`; (2) script ran `apps deploy` on a STOPPED app — now `apps start` first and the app deploy moved after schema/grants; (3) first `apps deploy` needs `--source-code-path` — now passed from `bundle summary`; (4) **the inline `apps.config.command`/`.env` in databricks.yml is NOT delivered to the Apps runtime by CLI v1.3.0** (only resource bindings are) → app came up with no command/env (503) — now `deploy_bundle.sh` **generates `app.yaml`** (command + resolved env) each deploy, gitignored, synced via `sync.include`-style re-deploy; (5) `ensure_databricks_auth` bailed with no `DATABRICKS_HOST` env (always true in a notebook) — now falls back to ambient `WorkspaceClient()` SDK auth; (6) `.env.shared` gitignored + not in `sync.include` → never synced — added to `sync.include`; (7) `from pgvector.psycopg import Vector` breaks on Serverless (Vector is top-level there) — import with fallback; (8) `mlflow.set_experiment("/Shared/agent-memory/…")` NOT_FOUND on parent dir — new `config.set_mlflow_experiment` mkdirs the parent (all 5 call sites); (9) `databricks_configured` read the frozen `Settings.databricks_host` (None in a notebook) → chat path falsely "Lakebase not configured" — now falls back to live env. Also folded `00_setup` into the deploy (auto-runs at the end; `SKIP_SETUP=1` to opt out) and the schema is created by both `deploy_bundle.sh` and `00_setup` (idempotent) so no manual `CREATE SCHEMA`. Gates: ruff + pyright(src) clean, 260 pass. INSTALL.md rewritten (00_setup now automatic; 01_seed/02_demo optional).
- 2026-06-10 — **Bundle-provisioned infra + single-source config (ADR-0016, NFR-2)** *(NOTE 2026-07-27: the "inline `config.env` honored by `apps deploy`" assumption below turned out FALSE on CLI v1.3.0 — see the 2026-07-27 entry; app command/env now come from a generated `app.yaml`, not the bundle's inline `config`)*: a new workspace no longer needs manual prereqs or multi-file edits. The bundle now **provisions** its own Lakebase instance (`database_instances`, `agent-memory-${target}`) + SQL warehouse (`sql_warehouses`); the app + jobs reference them via `${resources…read_write_dns/.id}` — all FEVM literals (host, profile, warehouse id, `ep-odd-term…` URL) removed. **`.env` is the single config source**: `deploy_bundle.sh` feeds it to the bundle (`BUNDLE_VAR_*`), generates the workspace-synced `.env.shared` (now gitignored) the notebooks read, and writes the resolved warehouse id + Lakebase endpoint back into `.env`. Root `app.yaml` **deleted** → app env is the bundle app resource's inline `config.env`, templated from `${var}`/`${resources}`. App↔Lakebase binding gives the app SP a Postgres role (runtime OAuth via the provisioned-instance path); notebooks derive the instance name from their target + look up the host via SDK; Lakebase table grants moved from the deploy script's `psql` block into `00_setup` (drops the local `psql` prereq). Job entry points gained `--lakebase-instance-name`. New `.env.example` (committed template); README quickstart rewritten for a fresh workspace. Gates: ruff + pyright(src) clean, 260 pass, `bundle validate` OK. **Not yet deployed live** — two runtime behaviors (inline `config.env` honored by `apps deploy`; `read_write_dns` via `${resources…}`) confirm only on a real `bundle deploy`; deploy script has a `get-database-instance` fallback for the host.
